...
/auth/me accepts the session cookie, the OAuth JWT, or the
X-API-Key header via the shared ``require_user`` dep. That is what
lets API-key callers (the CLI, service accounts, the robotic
platform) keep working unchanged while the web frontend
authenticates with cookies and OAuth.
